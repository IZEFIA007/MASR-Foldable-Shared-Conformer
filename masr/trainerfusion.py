...
class MASRTrainer(object):
        ...
        with model_context():
            for batch_id, batch in enumerate(self.train_loader):
                ...
                with torch.autocast('cuda', enabled=self.configs.train_conf.enable_amp):
                    # ==================== 修改开始：添加联合训练支持 ====================
                    # 检查配置中是否启用联合训练
                    joint_training_enabled = getattr(self.configs.train_conf, 'joint_training', False)

                    # 检查模型是否支持联合训练（兼容单卡和多卡）
                    model_supports_joint_training = False
                    if hasattr(self.model, 'module'):
                        # 多卡训练情况
                        if hasattr(self.model.module, 'joint_training'):
                            model_supports_joint_training = True
                    else:
                        # 单卡训练情况
                        if hasattr(self.model, 'joint_training'):
                            model_supports_joint_training = True

                    # 打印详细调试信息
                    if batch_id % self.configs.train_conf.log_interval == 0:
                        print(f"=== 联合训练调试信息 ===")
                        print(f"配置中的 joint_training: {joint_training_enabled}")
                        print(f"模型支持联合训练: {model_supports_joint_training}")
                        print(
                            f"实际使用的训练模式: {'联合训练' if (model_supports_joint_training and joint_training_enabled) else '单模型训练'}")

                    # 根据联合训练开关调用模型
                    if model_supports_joint_training and joint_training_enabled:
                        # 使用联合训练模式（model.forward 会返回包含 loss_kd 的 loss_dict）
                        loss_dict = self.model(inputs, input_lens, labels, label_lens, joint_training=True)
                    else:
                        # 使用普通训练模式
                        loss_dict = self.model(inputs, input_lens, labels, label_lens)

                    # Some fusion models already include ``kd_alpha * loss_kd`` in
                    # loss_dict['loss'].  Keep the historical extra raw-KL term by
                    # default, but allow new experiments to disable the duplicate
                    # addition without changing the model or its logging outputs.
                    add_raw_kd_in_trainer = self.configs.train_conf.get(
                        'add_raw_kd_in_trainer', True)
                    if add_raw_kd_in_trainer and 'loss_kd' in loss_dict:
                        # 保证 loss 字段存在且为标量 Tensor
                        if 'loss' in loss_dict and isinstance(loss_dict['loss'], torch.Tensor):
                            loss_dict['loss'] = loss_dict['loss'] + loss_dict['loss_kd']
                        else:
                            # fallback：若模型使用不同字段命名，可以把 loss_kd 当作 loss
                            loss_dict['loss'] = loss_dict['loss_kd']
                    # ====================================================================
                    # 打印返回的损失信息，检查联合训练是否生效
                    if batch_id % self.configs.train_conf.log_interval == 0:
                        print(f"返回的损失字典包含的键: {list(loss_dict.keys())}")

                        if 'loss_max' in loss_dict and 'loss_seed' in loss_dict:
                            print("[OK] 联合训练生效!")
                            print(f"   总联合损失: {loss_dict['loss']:.4f}")
                            print(f"   最大展开损失: {loss_dict['loss_max']:.4f}")
                            print(f"   种子模型损失: {loss_dict['loss_seed']:.4f}")
                            # 打印联合训练权重 alpha_p
                            if hasattr(self.model, 'alpha_p'):
                                print(f"   联合权重 alpha_p: {self.model.alpha_p}")
                            elif hasattr(self.model, 'module') and hasattr(self.model.module, 'alpha_p'):
                                print(f"   联合权重 alpha_p: {self.model.module.alpha_p}")
                            else:
                                print(f"   联合权重 alpha_p: N/A")

                            # ---- 新增：打印自蒸馏（KD）参数与损失 ----
                            if 'loss_kd' in loss_dict:
                                print(f"   蒸馏损失 loss_kd: {loss_dict['loss_kd']:.4f}")
                            else:
                                print(f"   蒸馏损失 loss_kd: 未返回（KD 未生效）")

                            # 从 model 或 model.module 中读取超参（若存在）
                            if hasattr(self.model, 'kd_alpha'):
                                print(f"   kd_alpha: {self.model.kd_alpha}")
                            elif hasattr(self.model, 'module') and hasattr(self.model.module, 'kd_alpha'):
                                print(f"   kd_alpha: {self.model.module.kd_alpha}")
                            else:
                                # 作为保险，从配置里读（可能你把 KD 放在 configs.model_conf.model_args）
                                kd_a = getattr(self.configs.model_conf.model_args, 'kd_alpha', None) \
                                    if hasattr(self.configs, 'model_conf') and hasattr(self.configs.model_conf,
                                                                                       'model_args') \
                                    else None
                                print(f"   kd_alpha: {kd_a if kd_a is not None else 'N/A'}")

                            if hasattr(self.model, 'kd_temperature'):
                                print(f"   kd_temperature: {self.model.kd_temperature}")
                            elif hasattr(self.model, 'module') and hasattr(self.model.module, 'kd_temperature'):
                                print(f"   kd_temperature: {self.model.module.kd_temperature}")
                            else:
                                kd_t = getattr(self.configs.model_conf.model_args, 'kd_temperature', None) \
                                    if hasattr(self.configs, 'model_conf') and hasattr(self.configs.model_conf,
                                                                                       'model_args') \
                                    else None
                                print(f"   kd_temperature: {kd_t if kd_t is not None else 'N/A'}")
                            # ---------------------------------------------
                        else:
                            print("[ERROR] 联合训练未生效")
                            print(f"   单模型损失: {loss_dict['loss']:.4f}")
                        print("=======================\n")
                    # ==================== 修改结束 ====================
                ...
                if batch_id % self.configs.train_conf.log_interval == 0 and self.local_rank == 0:
                    ...
                    # ==================== 修改开始：添加联合训练日志 ====================
                    # 构建日志消息
                    log_msg = f'Train epoch: [{epoch_id}/{self.configs.train_conf.max_epoch}], ' \
                              f'batch: [{batch_id}/{len(self.train_loader)}], '

                    # 检查是否使用联合训练
                    joint_training_enabled = getattr(self.configs.train_conf, 'joint_training', False)
                    model_supports_joint_training = hasattr(self.model, 'module') and hasattr(self.model.module,
                                                                                              'joint_training') or hasattr(
                        self.model, 'joint_training')

                    if joint_training_enabled and model_supports_joint_training and 'loss_max' in loss_dict and 'loss_seed' in loss_dict:
                        # 联合训练日志（新增：若存在 loss_kd 则一并打印）
                        if 'loss_kd' in loss_dict:
                            log_msg += f'joint_loss: {self.train_loss:.5f}, ' \
                                       f'loss_max: {loss_dict["loss_max"]:.5f}, ' \
                                       f'loss_seed: {loss_dict["loss_seed"]:.5f}, ' \
                                       f'loss_kd: {loss_dict["loss_kd"]:.5f}, '
                        else:
                            log_msg += f'joint_loss: {self.train_loss:.5f}, ' \
                                       f'loss_max: {loss_dict["loss_max"]:.5f}, ' \
                                       f'loss_seed: {loss_dict["loss_seed"]:.5f}, '

                        # 记录联合训练的各个损失到VisualDL（新增：loss_kd）
                        writer.add_scalar('Train/Loss_max', loss_dict["loss_max"], self.train_log_step)
                        writer.add_scalar('Train/Loss_seed', loss_dict["loss_seed"], self.train_log_step)
                        if 'loss_kd' in loss_dict:
                            # 如果 loss_kd 不是 Tensor（极少数情况），先转为 float
                            try:
                                writer.add_scalar('Train/Loss_kd', float(loss_dict["loss_kd"]), self.train_log_step)
                            except Exception:
                                writer.add_scalar('Train/Loss_kd', loss_dict["loss_kd"].item(), self.train_log_step)
                    else:
                        # 普通训练日志
                        log_msg += f'loss: {self.train_loss:.5f}, '
                    # ==================== 修改结束 ====================

                    log_msg += f'learning_rate: {self.scheduler.get_last_lr()[0]:>.8f}, ' \
    ...
    def create_data(self,
                    annotation_path='dataset1/annotation/',
                    num_samples=1000000,
                    max_test_manifest=10000,
                    is_merge_audio=False,
                    only_build_vocab=False,
                    save_audio_path='dataset1/audio/merge_audio',
                    max_duration=600):
        """创建数据列表和词汇表模型等前期数据处理，并生成 train.json / test.json / eval.json"""

        if is_merge_audio:
            logger.info('开始合并音频...')
            merge_audio(annotation_path=annotation_path,
                        save_audio_path=save_audio_path,
                        max_duration=max_duration,
                        target_sr=self.configs.dataset_conf.dataset.sample_rate)
            logger.info('合并音频已完成，原始音频文件和标注文件已自动删除')
    ...
    def evaluate(self, resume_model=None, display_result=False,
        ...
        self.model.eval()
        eval_model = self.model.module if isinstance(self.model,
        ...
        with torch.no_grad():
            for batch_id, batch in enumerate(tqdm(self.test_loader)):
                ...
                inputs, labels, input_lens, label_lens = batch
                inputs, labels, input_lens = inputs.to(self.device), labels.to(self.device), input_lens.to(self.device)

                # 计算 loss
                loss_dict = eval_model(
                    inputs, input_lens, labels, label_lens,
                    joint_training=False, logical_depth=12
                )
                loss_value = loss_dict['loss']

                # 自动检测 NaN
                if torch.isnan(loss_value):
                    print(f"[NaN Detected] Batch {batch_id}: loss is NaN!")
                encoder_outs, ctc_probs, ctc_lens = eval_model.get_encoder_out(inputs, input_lens, logical_depth=12)
                if torch.isnan(encoder_outs).any():
                    print(f"[NaN Detected] Batch {batch_id}: encoder_outs contains NaN!")
                if torch.isnan(ctc_probs).any():
                    print(f"[NaN Detected] Batch {batch_id}: ctc_probs contains NaN!")

                losses.append(loss_value.cpu().detach().numpy() / self.configs.train_conf.accum_grad)
                ...
                for label, out_string in zip(labels_str, out_strings):
                    error_rate = wer(label, out_string) if self.metrics_type == 'wer' else cer(label, out_string)
                    error_results.append(error_rate)
                    if display_result:
                        logger.info(f'实际标签为：{label}')
                        logger.info(f'预测结果为：{out_string}')
                        logger.info(
                            f'这条数据的{self.metrics_type}：{round(error_rate, 6)}, '
        ...
        loss = float(sum(losses) / len(losses)) if losses else -1
        error_result = float(sum(error_results) / len(error_results)) if error_results else -1
