...
class BiTransformerDecoder(nn.Module):
    ...
    def __init__(self,
                 ...
                 max_len: int = 5000):
        super().__init__()#这里就构建了一左一右的两个解码器
    ...
    def forward(
        ...
        Returns:
            (tuple): tuple containing:
                ...
                olens: (batch, )
        """
        l_x, _, olens = self.left_decoder(memory, memory_mask, ys_in_pad, ys_in_lens)#正向解码
        r_x = torch.zeros([1])
        if reverse_weight > 0.0:
            r_x, _, olens = self.right_decoder(memory, memory_mask, r_ys_in_pad, ys_in_lens)#逆向解码
        return l_x, r_x, olens
    #推理阶段，只用左向，因为是自回归生成，是基于上一步进行输出，无法做到反向
...
class TransformerDecoder(nn.Module):
    ...
    #用于逐步生成解码，主要在推理解码时用（比如beam search或者贪心采样）
    def forward_one_step(
            self,
            memory: torch.Tensor,
            memory_mask: torch.Tensor,
            tgt: torch.Tensor,#tgt 就是 Transformer 解码器里的 目标序列输入
            tgt_mask: torch.Tensor,
            cache: Optional[List[torch.Tensor]] = None,
    ) -> Tuple[torch.Tensor, List[torch.Tensor]]:
        """Forward one step.
...
class DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        if self.concat_after:#这里进入自注意力层了
