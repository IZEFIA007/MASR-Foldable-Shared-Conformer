...
class ConformerEncoder(nn.Module):
    ...
    def output_size(self) -> int:
        return self._output_size#告诉你编码器输出多大
    ...
    def forward(
        ...
        return xs, masks
    #针对流式场景，一次只处理一个chunk（音频片段），同时利用缓存加速和减少重复计算，支持增量推理。
    def forward_chunk(
            self,
            xs: torch.Tensor,
            offset: int,
    ...
    ) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:#这是流式处理的模块，我在参数里关掉的话应该暂时不用管
