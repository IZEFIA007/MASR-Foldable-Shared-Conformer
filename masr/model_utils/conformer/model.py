...
class ConformerModel(torch.nn.Module):
    ...
    def forward(
        ...
        # NaN 检测
            ...
            #print(f"[NaN Detected] forward: loss NaN, encoder_out NaN={torch.isnan(encoder_out).any()}, "
                  #f"CTC NaN={torch.isnan(loss_ctc).any() if loss_ctc is not None else 'N/A'}, "
                  #f"ATT NaN={torch.isnan(loss_att).any() if loss_att is not None else 'N/A'}")
    ...
    def _calc_att_loss(self,
        ...
        if torch.isnan(loss_att):
            print(f"[NaN Detected] _calc_att_loss: loss_att NaN, encoder_out NaN={torch.isnan(encoder_out).any()}, "
                  f"decoder_out NaN={torch.isnan(decoder_out).any()}, r_decoder_out NaN={torch.isnan(r_decoder_out).any()}")
