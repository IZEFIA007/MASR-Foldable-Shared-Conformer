...
class NoPositionalEncoding(nn.Module):
    ...
    def forward(self, x: torch.Tensor, offset: int = 0) -> Tuple[torch.Tensor, torch.Tensor]:
        pos_emb = torch.zeros(1, x.size(1), self.d_model, device=x.device)
        return self.dropout(x), pos_emb

    def position_encoding(self, offset: int, size: int) -> torch.Tensor:
        return torch.zeros(1, size, self.d_model, device=self.dropout.weight.device if hasattr(self.dropout, 'weight') else torch.device('cpu'))
