...
class MASRTrainer(object):
    def __init__(self,
                 ...
                 use_gpu=True,
                 metrics_type="cer",
                 decoder="attention_rescoring",#这应该只是一个类型表达
        ...
        if use_gpu:
            ...
            self.device = torch.device("cpu")#软硬件的检测
        self.log_level = log_level.upper()
        logger.remove()
        logger.add(sink=sys.stdout, level=self.log_level)#日志的处理，先不用管
        # 读取配置文件
        ...
        # 覆盖配置文件中的参数（这个overwrites可以在具体的执行文件，如train.py中看到）
        if overwrites:
        ...
        self.decoder = decoder#self就是保存一下初始值
        ...
        self.beam_search_decoder = None#初始全部设为none，方便后续构建。
        ...
        self.stop_train, self.stop_eval = False, False#这些none就是初始化
    #（帮训练器准备好训练和测试用的数据加载器（DataLoader），也就是模型训练和验证时从磁盘加载数据的“流水线”。）
    ...
    # 提取特征保存文件（专门负责对训练集和测试集的音频数据进行特征提取，并将特征保存成文件，同时生成新的数据列表文件。）
    ...
    #根据配置初始化并准备好模型，以及训练相关的优化器和学习率调度器。
    ...
    #是模型解码输出结果的核心逻辑，用来根据不同的解码策略，将模型的CTC输出概率或编码器输出转换成最终文本字符串。
    ...
    def create_data(self,
        ...
        # ========== 按 train.txt / test.txt / eval.txt 生成对应 json ==========
        ...
        def write_json(txt_path, json_path):
            with open(txt_path, 'r', encoding='utf-8') as f, \
                ...
                for line in f:
                    ...
                    if len(parts) < 3:
                        continue
                    audio, text, duration = parts[0], parts[1], float(parts[2])
                    jf.write(json.dumps({
    ...
    def train(self,
        ...
        for epoch_id in range(last_epoch, self.configs.train_conf.max_epoch):
            ...
            if self.local_rank == 0:
                ...
                # 早停逻辑（延迟早停）
                cer_threshold = 0.1  # CER 达到阈值才开始监控早停
                early_stop_patience = 5  # 连续多少轮没有提升就停止

                if not hasattr(self, 'best_eval'):
                    self.best_eval = float('inf')
                    self.no_improve_epochs = 0

                # 只在 CER 达到阈值时才更新早停计数
                if self.eval_error_result <= cer_threshold:
                    if self.eval_error_result < self.best_eval:
                        self.best_eval = self.eval_error_result
                        self.no_improve_epochs = 0
                    else:
                        self.no_improve_epochs += 1
                        logger.info(f'验证集指标未提升 {self.no_improve_epochs} 轮')

                    if self.no_improve_epochs >= early_stop_patience:
                        logger.info(f'验证集连续 {early_stop_patience} 轮未提升，触发早停')
                        self.stop_train = True
                else:
                    logger.info(f'验证集 CER {self.eval_error_result:.5f} 未达到阈值 {cer_threshold}, 不计入早停')
