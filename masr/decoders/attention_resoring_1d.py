...
def attention_rescoring(
    ...
    for b in range(batch_size):
        ...
        outs = model.get_decoder_out(hyps_pad, hyps_lens, encoder_out, reverse_weight)

        # Normalize possible return formats:
        #  - (decoder_out, r_decoder_out)
        #  - decoder_out  (single)
        #  - (decoder_out,) or similar
        if isinstance(outs, (list, tuple)) and len(outs) >= 2:
            decoder_out, r_decoder_out = outs[0], outs[1]
        else:
            decoder_out = outs if not (isinstance(outs, (list, tuple)) and len(outs) == 1) else outs[0]
            r_decoder_out = None
        ...
        for i, hyp in enumerate(hyps):
            score = 0.0
            # token scores from decoder_out
            for j, w in enumerate(hyp[0]):
                score += decoder_out[i][j][w]
            # last decoder output token is `eos`, for last decoder input token.
            score += decoder_out[i][len(hyp[0])][eos]

            # reverse rescoring only if r_decoder_out is provided and reverse_weight > 0
            if (r_decoder_out is not None) and (reverse_weight > 0):
                r_score = 0.0
                # ensure indices are valid
                for j, w in enumerate(hyp[0]):
                    # reverse index: len(hyp[0]) - j - 1
                    r_index = len(hyp[0]) - j - 1
                    r_score += r_decoder_out[i][r_index][w]
                r_score += r_
